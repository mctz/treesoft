...
public class PermissionController extends BaseController
{
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            {
                listTable = permissionService.getAllTablesForMongoDB(dbName, databaseConfigId);
            }
        }
        catch (Exception e)
        {
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            if (databaseType.equals("MongoDB"))
            {
                listDb = permissionService.getAllDataBaseForMongoDB(databaseConfigId);
            }
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            if (databaseType.equals("MongoDB"))
            {
                page = permissionService.getDataForMongoDB(page, tableName, dbName, databaseConfigId);
                result.put("operator", "edit");
            }
            if (databaseType.equals("Hive2"))
            {
    ...
    public Map<String, Object> executeSqlTest(@PathVariable String databaseConfigId, String sql, String databaseName)
    {
        ...
        if (databaseType.equals("MongoDB"))
        {
            if (StringUtils.startsWithAny(sql.toLowerCase(), "db.", "DB."))
            {
                if (StringUtils.contains(sql, "find"))
                {
                    result = executeSqlHaveResForMongoDB(sql, databaseName, databaseConfigId);
                }
                else if (StringUtils.containsAny(sql, "insert", "remove"))
                {
                    result = executeSqlNotResForMongoDB(sql, databaseName, databaseConfigId);
                }
                else
                {
                    result.put("mess", "暂未完成MongoDB该shell命令的解析执行!");
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            if (databaseType.equals("MongoDB"))
            {
                permissionService.deleteRowsNewForMongoDB(databaseName, tableName, primary_key, condition, databaseConfigId);
            }
    ...
    public String showTableData(@PathVariable String tableName, @PathVariable String databaseName, @PathVariable String databaseConfigId, @PathVariable String objectType)
    {
        ...
        if (databaseType.equals("MongoDB"))
        {
            return "system/showTableDataForMongo";
        }
        ...
        throws Exception
    {
        ...
        if (databaseType.equals("Hive2"))
        {
            result.put("mess", "暂不支持Hive");
            result.put("status", "fail");
        }
        if (databaseType.equals("MongoDB"))
        {
            result.put("mess", "暂不支持MongoDB");
            result.put("status", "fail");
        }
        ...
        try
        {
            if (inserted != null)
            {
                ...
                for (int i = 0; i < insertArray.size(); i++)
                {
                    ...
                    if (databaseType.equals("MongoDB"))
                    {
                        permissionService.saveRowsForMongoDB(maps, databaseName, tableName, databaseConfigId);
                    }
                }
            }
            List<String> condition = new ArrayList<>();
            if (updated != null)
            {
                ...
                for (int i = 0; i < updateArray.size(); i++)
                {
                    ...
                    else
                    {
                        ...
                        if (databaseType.equals("MongoDB"))
                        {
                            for (String primaryKey : primaryKeys)
                            {
                                whereStr = (String)map2.get(primaryKey);
                            }
                        }
                        else if (databaseType.equals("PostgreSQL"))
                        {
                ...
                if (databaseType.equals("MongoDB"))
                {
                    permissionService.updateRowsNewForMongoDB(databaseName, tableName, condition, databaseConfigId);
                    permissionService.saveLog("update " + databaseName + "." + tableName + " " + condition.toString(), username, ip);
                }
            }
            mess = "保存成功！";
    ...
    public Map<String, Object> executeSqlHaveResForMongoDB(String sql, String dbName, String databaseConfigId)
    {
        Map<String, Object> map = new HashMap<>();
        Page<Map<String, Object>> page = getPage(request);
        String mess = "";
        ...
        try
        {
            page = permissionService.executeSqlHaveResForMongoDB(page, sql, dbName, databaseConfigId);
            mess = "执行成功！";
            status = "success";
        }
        catch (Exception e)
    ...
    public Map<String, Object> executeSqlNotResForMongoDB(String sql, String dbName, String databaseConfigId)
    {
        ...
        try
        {
            i = permissionService.executeSqlNotResForMongoDB(sql, dbName, databaseConfigId);
            mess = "执行成功！";
            status = "success";
        }
        catch (Exception e)
    ...
    public Map<String, Object> queryDatabaseStatus(@PathVariable String databaseName, @PathVariable String databaseConfigId)
    {
        ...
        try
        {
            ...
            if (databaseType.equals("MongoDB"))
            {
                map = permissionService.queryDatabaseStatusForMongoDB(databaseName, databaseConfigId);
                mess = "查询成功";
                status = "success";
            }
    ...
    @ResponseBody
    public Map<String, Object> backupDatabaseData()
    {
        Map<String, Object> map = new HashMap<>();
        String path = request.getSession().getServletContext().getRealPath("/") + "backup";
        List<Map<String, Object>> list = permissionService.selectBackupList(path);
        map.put("rows", list);
        map.put("total", Integer.valueOf(list.size()));
        return map;
    }
    
    @RequestMapping(value = {"i/backupDatabaseExecute/{databaseConfigId}"}, method = {RequestMethod.POST})
    @ResponseBody
    public Map<String, Object> backupDatabaseExecute(@RequestBody IdsDto tem, @PathVariable String databaseConfigId)
        throws Exception
    {
        String databaseName = tem.getDatabaseName();
        Map<String, Object> mapResult = new HashMap<>();
        ...
        try
        {
            if (databaseType.equals("MySql"))
            {
                permissionService.backupDatabaseExecute(databaseName, "", path, databaseConfigId);
                Thread.sleep(3000L);
                mess = "备份完成！";
                status = "success";
            }
            ...
            if (databaseType.equals("Oracle"))
            {
                permissionService.backupDatabaseExecuteForOracle(databaseName, "", path, databaseConfigId);
                Thread.sleep(3000L);
                mess = "备份完成！";
                status = "success";
            }
            if (databaseType.equals("PostgreSQL"))
            {
                permissionService.backupDatabaseExecuteForPostgreSQL(databaseName, "", path, databaseConfigId);
                Thread.sleep(3000L);
                mess = "备份完成！";
                status = "success";
            }
            if (databaseType.equals("MSSQL"))
            {
                permissionService.backupDatabaseExecuteForMSSQL(databaseName, "", path, databaseConfigId);
                Thread.sleep(3000L);
                mess = "备份完成！";
                status = "success";
            }
    ...
    @RequestMapping(value = {"i/exportTable/{databaseConfigId}"}, method = {RequestMethod.POST})
    @ResponseBody
    public Map<String, Object> exportTable(@RequestBody IdsDto tem, @PathVariable String databaseConfigId)
        throws Exception
    {
        String databaseName = tem.getDatabaseName();
        String tableName = tem.getTableName();
        ...
        try
        {
            if (databaseType.equals("MySql"))
            {
                permissionService.backupDatabaseExecute(databaseName, tableName, path, databaseConfigId);
                Thread.sleep(3000L);
                mess = "导出表完成，请刷新并下载！";
                status = "success";
            }
            ...
            if (databaseType.equals("Oracle"))
            {
                permissionService.backupDatabaseExecuteForOracle(databaseName, tableName, path, databaseConfigId);
                Thread.sleep(3000L);
                mess = "导出表完成，请刷新并下载！";
                status = "success";
            }
            if (databaseType.equals("PostgreSQL"))
            {
                permissionService.backupDatabaseExecuteForPostgreSQL(databaseName, tableName, path, databaseConfigId);
                Thread.sleep(3000L);
                mess = "导出表完成，请刷新并下载！";
                status = "success";
            }
            if (databaseType.equals("MSSQL"))
            {
                permissionService.backupDatabaseExecuteForMSSQL(databaseName, tableName, path, databaseConfigId);
                Thread.sleep(3000L);
                mess = "导出表完成，请刷新并下载！";
                status = "success";
            }
    ...
    @RequestMapping(value = {"i/exportDataToSQL/{databaseConfigId}"}, method = {RequestMethod.POST})
    @ResponseBody
    public void exportDataToSQL(IdsDto tem, @PathVariable String databaseConfigId, HttpServletResponse response)
        throws Exception
    {
        String databaseName = tem.getDatabaseName();
        String tableName = tem.getTableName();
        ...
        if (checkedItems != null)
        {
            ...
            for (int i = 0; i < deleteArray.size(); i++)
            {
                if (databaseType.equals("MongoDB"))
                {
                    ...
                    ids = obj.getString("_id");
                    condition.add(ids);
                }
                else
                {
                    map1 = (Map)deleteArray.get(i);
                    String whereStr = "";
                    if ((primary_key == null) || (primary_key.equals("")))
                    {
                        for (String key : map1.keySet())
                        {
                            if ((!databaseType.equals("Oracle")) || (!key.equals("RN")))
                            {
        ...
        if (databaseType.equals("MySql"))
        {
            listTableColumn = permissionService.getTableColumns3(databaseName, tableName, databaseConfigId);
        }
        if (databaseType.equals("MariaDB"))
        {
            listTableColumn = permissionService.getTableColumns3(databaseName, tableName, databaseConfigId);
        ...
        if (databaseType.equals("Oracle"))
        {
            listTableColumn = permissionService.getTableColumns3ForOracle(databaseName, tableName, databaseConfigId);
        }
        if (databaseType.equals("PostgreSQL"))
        {
            listTableColumn = permissionService.getTableColumns3ForPostgreSQL(databaseName, tableName, databaseConfigId);
        }
        if (databaseType.equals("MSSQL"))
        {
            listTableColumn = permissionService.getTableColumns3ForMSSQL(databaseName, tableName, databaseConfigId);
        }
        ...
        try
        {
            if (databaseType.equals("MySql"))
            {
                dataList = permissionService.exportDataToSQLForMySQL(databaseName, tableName, condition, databaseConfigId);
            }
            if (databaseType.equals("MariaDB"))
            {
                dataList = permissionService.exportDataToSQLForMySQL(databaseName, tableName, condition, databaseConfigId);
            ...
            if (databaseType.equals("Oracle"))
            {
                dataList = permissionService.exportDataToSQLForOracle(databaseName, tableName, condition, databaseConfigId);
            }
            if (databaseType.equals("PostgreSQL"))
            {
                dataList = permissionService.exportDataToSQLForPostgreSQL(databaseName, tableName, condition, databaseConfigId);
            }
            if (databaseType.equals("MSSQL"))
            {
                dataList = permissionService.exportDataToSQLForMSSQL(databaseName, tableName, condition, databaseConfigId);
            }
            if (databaseType.equals("MongoDB"))
            {
                dataList = permissionService.exportDataToSQLForMongoDB(databaseName, tableName, condition, databaseConfigId);
            }
            String tempInsertSqlString = "";
            if (databaseType.equals("MySql"))
            {
                tempInsertSqlString = dataListToStringForMySQL(tableName, TableColumnType, dataList);
            }
            if (databaseType.equals("MariaDB"))
            {
                tempInsertSqlString = dataListToStringForMySQL(tableName, TableColumnType, dataList);
            ...
            if (databaseType.equals("Oracle"))
            {
                tempInsertSqlString = dataListToStringForOracle(tableName, TableColumnType, dataList);
            }
            if (databaseType.equals("PostgreSQL"))
            {
                tempInsertSqlString = dataListToStringForPostgreSQL(tableName, TableColumnType, dataList);
            }
            if (databaseType.equals("MSSQL"))
            {
                tempInsertSqlString = dataListToStringForMSSQL(tableName, TableColumnType, dataList);
            }
            if (databaseType.equals("MongoDB"))
            {
                tempInsertSqlString = dataListToStringForMongoDB(dataList);
            }
            response.setContentType("multipart/form-data");
            response.addHeader("Content-Disposition", "attachment;fileName=" + tableName + ".sql ");
            try
            {
                InputStream inputStream = new ByteArrayInputStream(tempInsertSqlString.getBytes());
        ...
        throws Exception
    {
        ...
        try
        {
            if ((tableName != null) || (!"".equals(tableName)))
            {
                if (databaseType.equals("MongoDB"))
                {
                    bool = permissionService.dropTableForMongoDB(databaseName, tableName, databaseConfigId);
                }
        ...
        throws Exception
    {
        ...
        try
        {
            if (databaseType.equals("MongoDB"))
            {
                bool = permissionService.dropDatabaseForMongoDB(databaseName, databaseConfigId);
            }
            else
            {
        ...
        throws Exception
    {
        ...
        try
        {
            if ((tableName != null) || (!"".equals(tableName)))
            {
                if (databaseType.equals("MongoDB"))
                {
                    bool = permissionService.clearTableForMongoDB(databaseName, tableName, databaseConfigId);
                }
                else
                {
        ...
        throws Exception
    {
        ...
        if (databaseType.equals("MongoDB"))
        {
            listAllColumn = permissionService.viewTableMessForMongoDB(databaseName, tableName, databaseConfigId);
        }
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            if (databaseType.equals("MongoDB"))
            {
                permissionService.saveNewTableForMongoDB(insertArray, databaseName, tableName, databaseConfigId);
            }
            mess = "保存成功！";
            status = "success";
        }
    ...
    @RequestMapping(value = {"i/exportDataToSQLFromSQL"}, method = {RequestMethod.POST})
    @ResponseBody
    public void exportDataToSQLFromSQL(HttpServletResponse response)
        throws Exception
    {
        ...
        try
        {
            ...
            if (databaseType.equals("MongoDB"))
            {
                dataList = permissionService.selectAllDataFromSQLForMongoDBForExport(databaseName, databaseConfigId, sql);
            }
            if (exportType.equals("json"))
            {
                ...
                if (databaseType.equals("MySql"))
                {
                    tempInsertSqlString = dataListToTxt(dataList);
                }
                if (databaseType.equals("MariaDB"))
                {
                    tempInsertSqlString = dataListToTxt(dataList);
            ...
            {
                ExportExcelUtils.downloadExcel(response, dataList);
            }
        }
        catch (Exception e)
        {
    ...
    @RequestMapping(value = {"i/dataSynchronizeUpdate"}, method = {RequestMethod.POST})
    @ResponseBody
    public Map<String, Object> dataSynchronizeUpdate(@ModelAttribute @RequestBody DataSynchronize dataSynchronize, Model model)
    {
        ...
        try
        {
            String dataSynchronizeId = dataSynchronize.getId();
            ...
            permissionService.dataSynchronizeUpdate(dataSynchronize);
            if (!dataSynchronizeId.equals(""))
            {
                if (status2.equals("1"))
                {
                    Map<String, Object> job = permissionService.getDataSynchronizeById2(dataSynchronizeId);
                    QuartzManager.removeJob(dataSynchronizeId);
                    quartzManager.addJob(dataSynchronizeId, QuartzJobFactory.class, cron, job);
                }
                if (state.equals("1"))
                {
                    QuartzManager.removeJob(dataSynchronizeId);
    ...
    @RequestMapping(value = {"i/deleteDataSynchronize"}, method = {RequestMethod.POST})
    @ResponseBody
    public Map<String, Object> deleteDataSynchronize(@RequestBody IdsDto tem)
    {
        String[] ids = tem.getIds();
        String mess = "";
        String status = "";
        try
        {
            permissionService.deleteDataSynchronize(ids);
            for (String dataSynchronizeId : ids)
            {
                QuartzManager.removeJob(dataSynchronizeId);
                permissionService.deleteDataSynchronizeLogByDS(dataSynchronizeId);
            }
            mess = "删除成功";
            status = "success";
        }
    ...
    @RequestMapping(value = {"i/startDataTask"}, method = {RequestMethod.POST})
    @ResponseBody
    public Map<String, Object> startDataTask(@RequestBody IdsDto tem)
    {
        String[] ids = tem.getIds();
        String mess = "";
        String status = "";
        ...
        try
        {
            List<Map<String, Object>> list = permissionService.getDataSynchronizeListById(ids);
            for (Map<String, Object> job : list)
            {
                ...
                QuartzManager.removeJob(dataSynchronizeId);
                quartzManager.addJob(dataSynchronizeId, QuartzJobFactory.class, job.get("cron").toString(), job);
                permissionService.dataSynchronizeUpdateStatus(dataSynchronizeId, "1");
            }
            mess = "任务已提交!";
            status = "success";
        }
        ...
        {
            logger.error(e.getMessage(), e);
            permissionService.dataSynchronizeUpdateStatus(dataSynchronizeId, "0");
            mess = e.getMessage();
            status = "fail";
        }
        Map<String, Object> map = new HashMap<>();
    ...
    @RequestMapping(value = {"i/startDataTaskOne"}, method = {RequestMethod.POST})
    @ResponseBody
    public Map<String, Object> startDataTaskOne(@RequestBody IdsDto tem)
    {
        String[] ids = tem.getIds();
        String mess = "";
        String status = "";
        ...
        try
        {
            ...
            {
                dataSynchronizeId = (String)job.get("id");
                quartzManager.addJobOne(dataSynchronizeId, QuartzJobFactory.class, job);
            }
            mess = "任务已提交!";
            status = "success";
        }
    ...
    @RequestMapping(value = {"i/stopDataTask"}, method = {RequestMethod.POST})
    @ResponseBody
    public Map<String, Object> stopDataTask(@RequestBody IdsDto tem)
    {
        String[] ids = tem.getIds();
        String mess = "";
        String status = "";
        ...
        throws Exception
    {
        ...
        try
        {
            page = permissionService.getDataForMongoJson(page, tableName, dbName, databaseConfigId);
