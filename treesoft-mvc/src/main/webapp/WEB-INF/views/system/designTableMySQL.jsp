...
<body>
  <div style="padding:0px;height:600px">
   ...
   <div id="designTableTab" class="easyui-tabs"  data-options="fit: true, border: false" >
      ...
      <div  data-options="title: '外键', iconCls: 'icon-hamburg-invoice'"  >
           <div id="toolbarForeignKey">
           <div> 
            <span class="toolbar-item dialog-tool-separator"></span>
	        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-table-row-delete" plain="true" id="delRowButton"   onclick="deleteForeignKey()">删除外键</a>
	        <span class="toolbar-item dialog-tool-separator"></span>
	         
           </div>
          </div>               
		  <table id="dgForForeignKey"></table> 	
      </div>
...
<script type="text/javascript">
  ...
  function initIndexData(){
    ...
    columns:[[    
        ...
        {field:'Non_unique',title:'索引类型',halign:'center',width:100,editor:{type:'text', options: { required: true }},formatter: function(value,row,index){
				if (row.Non_unique =='0'){
					return 'Unique';
        ...
        {field:'Index_type',title:'索引算法',width:100, editor:{
                    	 type: 'combobox',
                         options: {
                            data: IndexTypes,
                            valueField: "value",
  ...
  function initForeignKeyData(){
     $("#dgForForeignKey").datagrid({     
	    method: "post",
        url: '${ctx}/designTable/selectTableForeignKey', 
        queryParams:{databaseConfigId:databaseConfigId,databaseName:databaseName,tableName:tableName },
        fit : true,
	    fitColumns : true,
	    border : false,
        ...
        columns:[[    
	    	{field:'TREESOFTPRIMARYKEY',checkbox:true}, 		  
	    	{field:'constraint_name',title:'外键名称', width:30   },		
	    	{field:'column_name',title:'列名称',width:50   } ,
	    	{field:'referenced_table_schema',title:'引用数据库',width:50   } ,
         	{field:'referenced_table_name',title:'引用表名称',width:50   } ,
         	{field:'referenced_column_name',title:'引用列名称',width:50   }
         	 	
        ]], 
        checkOnSelect:true,
        selectOnCheck:true,
        extEditing:false,
        toolbar:'#toolbarForeignKey', 
  ...
  function initMessData(){
   ...
   function deleteForeignKey(){
	  ...
	  var checkedItems = $("#dgForForeignKey").datagrid('getChecked');
	  ...
	  $.easyui.messager.confirm("操作提示", "您确定要删除"+length+"行数据吗？", function (c) {
                if (c) {                           	
                   $.ajax({
                    ...
                    url:"${ctx}/designTable/deleteForeignKey",
                    data:JSON.stringify({"databaseConfigId":databaseConfigId,"databaseName":databaseName,"tableName":tableName,"foreignKeyName":checkedItems[0]['constraint_name'] }),
                    datatype: "json", 
                    cache:false,
                   //成功返回之后调用的函数             
                    success:function(data){
            	       ...
            	       if(status == 'success' ){
            	    	   $("#dgForForeignKey").datagrid('reload');
            	    	   $("#dgForForeignKey").datagrid('clearSelections').datagrid('clearChecked');
 ...
 //删除 触发器 
  function deleteTrigger(){
	  ...
	  triggerName = checkedItems[0]['TRIGGER_NAME'] ;
